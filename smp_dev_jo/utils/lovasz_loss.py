...
def criterion_lovasz_softmax_non_empty(criterion, logits_deep, y):
    batch,h,w = y.size()
    y2 = y.view(batch,-1)
    logits_deep2 = logits_deep.view(batch, 11, -1)
    ...
    if non_empty_idx.sum()==0:
        ...
        loss  = criterion(logits_deep2[non_empty_idx], 
                          y2[non_empty_idx])
        loss += lovasz_softmax(logits_deep2[non_empty_idx].view(-1,11,h,w), 
